...
const BitacoraMensual = () => {
    ...
    const initializeRegistros = (numDays) => {
        return Array(numDays).fill({ malware: 0, phishing: 0, comando_y_control: 0, criptomineria: 0, denegacion_de_servicios: 0, intentos_de_conexion: 0 });
    };
    ...
    const fetchBitacoraData = useCallback(async (selectedMes, selectedYear) => {
        ...
        setIsLoading(true);
        try {
            const response = await axios.get('https://192.168.39.115/gestion-incidentes/backend/bitacora_mensual.php', { params: { mes: selectedMes, year: selectedYear } });
            ...
            if (response.data.success && response.data.registros.length > 0) {
                ...
                const newRegistros = initialRegistros.map((_, index) => {
                    const day = index + 1;
                    // FIX: Parse day from date string to avoid timezone issues
                    const found = fetchedData.find(d => parseInt(d.fecha.split('-')[2], 10) === day);
                    return found ? { malware: parseInt(found.malware) || 0, phishing: parseInt(found.phishing) || 0, comando_y_control: parseInt(found.comando_y_control) || 0, criptomineria: parseInt(found.criptomineria) || 0, denegacion_de_servicios: parseInt(found.denegacion_de_servicios) || 0, intentos_de_conexion: parseInt(found.intentos_de_conexion) || 0 } : initialRegistros[index];
                });
